        ...
        $response->assertOk()
            ->assertJson([
                ...
                'links' => [
                    ...
                    'prev'  => null,
                    'next'  => null,
                    'self'  => \sprintf('%s/api/torrents', config('app.url')),
                ],
                'meta' => [
                    'path'        => \sprintf('%s/api/torrents/filter', config('app.url')),
                    'per_page'    => 25,
                    'next_cursor' => null,
                    'prev_cursor' => null,
                ],
    ...
    public function index_returns_an_ok_response(): void
    {
        ...
        $response->assertOk()
            ->assertJson([
                'data'  => [],
                'links' => [
                    'first' => \sprintf('%s/api/torrents?page=1', config('app.url')),
                    'last'  => \sprintf('%s/api/torrents?page=1', config('app.url')),
                    'prev'  => null,
                    'next'  => null,
                    'self'  => \sprintf('%s/api/torrents', config('app.url')),
                ],
                'meta' => [
                    ...
                    'from'         => null,
                    'last_page'    => 1,
                    'path'         => \sprintf('%s/api/torrents', config('app.url')),
                    'per_page'     => 25,
                    'to'           => null,
                    'total'        => 0,
                ],
